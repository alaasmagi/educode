...
public class CourseRepository(AppDbContext context, ILogger<CourseRepository> logger, SentryService sentry) : ICourseRepository
{
    ...
    public async Task<List<(Guid, DateTime, int)>?> GetUserCountsAsync(Guid id)
    {
        try
        {
            var result = await context.Attendances
                ...
                .Select(a => new
                {
                    a.Id,
                    a.StartTime,
                    Count = context.AttendanceChecks.Count(ac => ac.AttendanceIdentifier == a.Identifier)
                })
                .ToListAsync();
            
            return result.Select(r => (r.Id, r.StartTime, r.Count)).ToList();
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<CourseEntity>?> GetAllSingleUserByUserAsync(Guid userId, bool includeDeleted)
    {
        try
        {
            return includeDeleted ?
        ...
        {
            logger.LogError(ex, "Error getting single user courses by user. User ID: {UserId}", userId);
            sentry.CaptureWithContext(ex, "Error getting single user courses by user. User ID: {0}", userId);
            return null;
        }
    }
    ...
    public async Task<List<Guid>?> GetAllIdsByUserAsync(Guid userId)
    {
        try
        {
            return await context.Courses
                .IgnoreQueryFilters()
                .Where(c => c.Teachers!.Any(ct => ct.TeacherId == userId))
                .Select(c => c.Id)
                .ToListAsync();
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting all course IDs by user. User ID: {UserId}", userId);
            sentry.CaptureWithContext(ex, "Error getting all course IDs by user. User ID: {0}", userId);
            return null;    
        }
    }
    
    public async Task<List<Guid>?> GetAllSingleUserCourseIdsByUserAsync(Guid userId)
    {
        try
        {
            return await context.Courses
                .Where(c => c.Teachers!.Count == 1 && c.Teachers.Any(ct => ct.TeacherId == userId))
                .Select(c => c.Id)
                .ToListAsync();
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting single user course IDs by user. User ID: {UserId}", userId);
            sentry.CaptureWithContext(ex, "Error getting single user course IDs by user. User ID: {0}", userId);
            return null;    
        }
    }
    ...
    public async Task<bool> ToggleDeletionAsync(Guid id, bool newDeletionState)
    {
        ...
        {
            logger.LogError(ex, "Error toggling deletion state for course. ID: {Id}, New State: {NewState}", 
                                                                                                    id, newDeletionState);
            sentry.CaptureWithContext(ex, "Error toggling deletion state for course. ID: {0}, New State: {1}", 
                                                                                                    id, newDeletionState);
            return false;
        }
    }
