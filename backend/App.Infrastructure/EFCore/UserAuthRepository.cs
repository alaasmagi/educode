...
public class UserAuthRepository(AppDbContext context, ILogger<UserAuthRepository> logger, SentryService sentry) : IUserAuthRepository
{
    ...
    public async Task<bool> ToggleDeletionAsync(Guid id, bool newDeletionState)
    {
        try
        {
            var affectedRows = await context.UserAuthData
                .IgnoreQueryFilters()
                .Where(ua => ua.Id == id)
        ...
        {
            logger.LogError(ex, "Error toggling deletion state for user auth data. ID: {Id}, New State: {NewState}", 
                id, newDeletionState);
            sentry.CaptureWithContext(ex, "Error toggling deletion state for user auth data. ID: {0}, New State: {1}", 
                id, newDeletionState);
            return false;
        }
    }
