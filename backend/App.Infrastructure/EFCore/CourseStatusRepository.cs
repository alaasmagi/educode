                                                                                                ...
                                                                                                : ICourseStatusRepository
{
    ...
    public async Task<CourseStatusEntity?> GetByItself(string statusName, bool includeDeleted)
    {
        try
        {
            return includeDeleted ? 
        ...
        {
            logger.LogError(ex, "Error retrieving course status. CourseStatus name: {Name}", statusName);
            sentry.CaptureWithContext(ex, "Error retrieving course status. CourseStatus name: {0}", statusName);
            return null;
        }
    }
    ...
    public async Task<bool> ToggleDeletionAsync(Guid id, bool newDeletionState)
    {
        try
        {
            var affectedRows = await context.CourseStatuses
                .IgnoreQueryFilters()
                .Where(cs => cs.Id == id)
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(cs => cs.Deleted, newDeletionState)
                    .SetProperty(cs => cs.UpdatedAt, DateTime.UtcNow));

            return affectedRows > 0;
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error toggling deletion state for course status. ID: {Id}, New State: {NewState}", 
                id, newDeletionState);
            sentry.CaptureWithContext(ex, "Error toggling deletion state for course status. ID: {0}, New State: {1}", 
                id, newDeletionState);
            return false;
        }
    }
