...
public class WorkplaceRepository(AppDbContext context, ILogger<WorkplaceRepository> logger, SentryService sentry) : IWorkplaceRepository
{
    ...
    public async Task<bool> ToggleDeletionAsync(Guid id, bool newDeletionState)
    {
        try
        {
            var affectedRows = await context.Workplaces
                .IgnoreQueryFilters()
                .Where(w => w.Id == id)
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(w => w.Deleted, newDeletionState)
        ...
        {
            logger.LogError(ex, "Error toggling deletion state for workplace. ID: {Id}, New State: {NewState}", 
                id, newDeletionState);
            sentry.CaptureWithContext(ex, "Error toggling deletion state for workplace. ID: {0}, New State: {1}", 
                id, newDeletionState);
            return false;
        }
    }
