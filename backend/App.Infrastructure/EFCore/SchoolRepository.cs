...
{ 
    ...
    public async Task<bool> ToggleDeletionAsync(Guid id, bool newDeletionState)
    {
        try
        {
            var affectedRows = await context.Schools
                .IgnoreQueryFilters()
                .Where(s => s.Id == id)
                .ExecuteUpdateAsync(setters => setters
                    .SetProperty(s => s.Deleted, newDeletionState)
        ...
        {
            logger.LogError(ex, "Error toggling deletion state for school. ID: {Id}, New State: {NewState}", 
                id, newDeletionState);
            sentry.CaptureWithContext(ex, "Error toggling deletion state for school. ID: {0}, New State: {1}", 
                id, newDeletionState);
            return false;
        }
    }
